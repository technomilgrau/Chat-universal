print("[Chat Universal] Iniciando script...")

local RENDER_WEBSOCKET_URL = "wss://chat-universal-online.onrender.com" -- Link atualizado!
...
print("[Chat Universal] Conectando ao WebSocket...")
...
if WebSocketApi then
    ...
    if not success or not ws then
        print("[Chat Universal] Erro ao conectar ao WebSocket. O Render pode estar offline ou seu executor não suporta WebSockets.")
        warn(err)
    else
        print("[Chat Universal] WebSocket conectado com sucesso!")
    ...
    print("[Chat Universal] SEU EXECUTOR NÃO SUPORTA WEBSOCKETS! (API WebSocket não encontrada)")
end
...
print("[Chat Universal] Interface carregada com sucesso!")
